...
def test2(done): # fails rn
    ...
    replace.data.update_all_cells_in_column(iv,iv_input)
    replace.data.update_all_cells_in_column(dv,dv_input)

    # run replacer and get new dv column
    replace.replace_iv_dv()

    dv_output = replace.data.return_column_as_list(dv)
    ...
    replace.data.update_all_cells_in_column(iv,empty_list)
    replace.data.update_all_cells_in_column(dv,empty_list)
